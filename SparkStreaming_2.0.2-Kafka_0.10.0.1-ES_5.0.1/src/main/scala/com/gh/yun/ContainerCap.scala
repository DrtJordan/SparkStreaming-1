...
import com.gh.utils.{AlertInfoUtil, HttpUtil, JsonUtil}
...
object ContainerCap {
  def main(args: Array[String]) {
   /* if (args.length < 4) {
    ...
    val sparkConf = new SparkConf().setAppName("capability-container").setMaster("local[2]")
    val ssc = new StreamingContext(sparkConf, Seconds(5))
    ssc.checkpoint("D:\\tmp\\checkpoint3")
 ...
 /* def parseLog(stream : InputDStream[ConsumerRecord[String, String]]) : DStream[(String,(String,Double,Double,Double,Double))] ={
    val datas = stream.map(line => {
      ...
      node
        ...
        val data_0 = node.get("data").get(0)

        val environment_id = data_0.get("container_labels").get("io.rancher.environment.id").asText()
        val container_uuid = data_0.get("container_labels").get("io.rancher.container.uuid").asText()
...
/*
  ...
  def compute(mysql_group : DStream[(String,Iterable[(String,Double,Double,Double,Double)])]): Unit ={
