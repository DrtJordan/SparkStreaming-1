...
//    })
  ...
  def ParseAppSaveToEs(datas : DStream[(String,JsonNode)]): Unit ={
    ...
    app.flatMap(lines => {
      ...
      arr
    }).saveJsonToEs("yun-{data.environment_id}-{data.container_uuid}-app-{type}-{data.stats.timestamp:YYYY-MM-dd}/{type}-cap",Map("es.batch.size.bytes"->"1048576","es.batch.size.entries"->"500","es.batch.write.refresh"->"false"))
  }
  ...
  def ParseContainerSaveToEs(datas : DStream[(String,JsonNode)]): Unit ={
    ...
    }).saveJsonToEs("yun-{data.environment_id}-{data.container_uuid}-container-container-{data.stats.timestamp:YYYY-MM-dd}/container-cap",Map("es.batch.size.bytes"->"1048576","es.batch.size.entries"->"500","es.batch.write.refresh"->"false"))
